...
// prototype for user source function
void GeneralNewtonianPotential(Mesh* pm, const Real dt);
...
// Function to add  user defined genral Newtonina potential as Tejeda & Rosswog 2013. The terms are in Caretian, Minkowski space-time to mimick non-GR, pseudo-Newtonian case.
void GeneralNewtonianPotential(Mesh* pm, const Real dt){
  std::cout<<"called user source"<<std::endl;
  // sanity check
  bool &flat = pm->pmb_pack->pcoord->coord_data.is_minkowski;
  if (!flat){
    std::cout << "### warning in " << __FILE__ << " at line " << __LINE__
              << std::endl << "User defined pseudo-Newtonian variable is only added in flat space-time" << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  const Real r_excise = pm->pmb_pack->pcoord->coord_data.rexcise;
  
  par_for("generalnewtonian", DevExeSpace(),0,(nmb-1),ks,ke,js,je,is,ie,
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    // get coordinate
    Real &x1min = size.d_view(m).x1min;
    Real &x1max = size.d_view(m).x1max;
    ...
    Real dx = w0_(m,IVX,k,j,i);
    Real dy = w0_(m,IVY,k,j,i);
    Real dz = w0_(m,IVZ,k,j,i);
    Real x = x1v;
    Real y = x2v;
    Real z = x3v;
    ...
    if (r > r_excise){
      ...
      //adding momentum source term, do not change internal energy
      //printf("in user source, x:%g, y:%g, z:%g, ax:%g, ay:%g, az:%g\n", x, y, z, ax, ay, az);
      w0_(m,IM1,k,j,i) += dt * ax ;
      w0_(m,IM2,k,j,i) += dt * ay ;
      w0_(m,IM3,k,j,i) += dt * az ;
      
      //u0_(m,IM1,k,j,i) += dt * rho * ax;
